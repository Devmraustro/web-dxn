...
// NOTE: the missing-MONGODB_URI fail-fast intentionally lives in db.connectDB
// (when a connection is actually attempted), NOT at module import: module-load
// time must stay side-effect free so static/health paths boot even when the
// database env var is absent. JWT_SECRET below keeps its import-time guard.
...
/** Default shipping fee (DA) used when no ShippingRate row exists. */
export const DEFAULT_SHIPPING_HOME = (() => {
  const n = Number(process.env.DEFAULT_SHIPPING_HOME);
  return Number.isFinite(n) && n >= 0 ? n : 0;
})();
export const DEFAULT_SHIPPING_OFFICE = (() => {
